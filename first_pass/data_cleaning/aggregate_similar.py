...
all_files = glob.glob(os.path.join(path, "similar_run_*.csv"))
all_files.sort()
...
for filename in all_files:
    try:
        ...
        match = re.search(r'similar_run_(\d+)_to_(\d+)', filename)
        if not match:
        ...
        for persona_id, persona_group in filtered_df.groupby('persona_id'):
            persona_group = persona_group[persona_group['comparison_number'].between(1, 50)]  # Ensure valid range
            ...
            for comp_num in range(1, 51):
                ...
                if not comp_rows.empty:
                    comparison_rows.append(comp_rows.iloc[0:1])  # Select only the first row

            # Combine selected rows for this persona
            if comparison_rows:
                persona_df = pd.concat(comparison_rows, ignore_index=True)
                # Ensure we don't keep duplicates in the full dataset
                persona_df.drop_duplicates(subset=['persona_id', 'comparison_number'], inplace=True)
...
print(f"Unique personas: {combined_df['persona_id'].nunique()}")
print(f"Total rows: {len(combined_df)}")
print(f"Comparison number distribution:\n{combined_df['comparison_number'].value_counts().sort_index()}")
...
output_file = "cleaned_results/combined_similar.csv"
combined_df.drop_duplicates(subset=['persona_id', 'comparison_number'], inplace=True)
print(f"Saving {combined_df.shape[0]} rows to CSV...")  # Final check before saving

combined_df.to_csv(output_file, index=False, mode='w', lineterminator='\n')

print(f"CSV saved! Check with: wc -l {output_file}")
