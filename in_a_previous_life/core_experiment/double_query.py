...
class FairnessConstraintGenerator:
    ...
    def extract_choice_and_explanation(self, response_text):
        ...
        # Check for option in the response
        ...
        # If no number at the beginning, try other patterns
        if chosen_option is None:
            if "option 1" in response_text.lower() or "first option" in response_text.lower():
                ...
                chosen_option = 3
            elif "x should have a higher" in response_text.lower() or "person x should have a higher" in response_text.lower():
                chosen_option = 1
    ...
    def elicit_fairness_judgments(self, train_df, personas_df, pairs_per_persona=50, start_comparison=0, end_comparison=None):
        ...
        for persona_idx, (_, persona_row) in enumerate(personas_df.iterrows()):
            ...
            for comparison_idx in range(start_comparison, end_comparison + 1):
                ...
                response1 = self.llama_query(prompt_list1, max_tokens=256, temperature=0.7)
                
                # Extract chosen option and explanation
                chosen_option1, explanation1 = self.extract_choice_and_explanation(response1)
                ...
                response2 = self.llama_query(prompt_list2, max_tokens=256, temperature=0.7)
                
                # Extract chosen option and explanation
                chosen_option2, explanation2 = self.extract_choice_and_explanation(response2)
