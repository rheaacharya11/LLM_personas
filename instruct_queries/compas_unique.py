...
def load_first_n_personas_from_unique_parquet(n=3) -> List[Tuple[int, str]]:
    """
    Load the first n personas from the unique_personas.parquet file
    
    Args:
        n: Number of personas to load from the beginning of the file
    ...
    try:
        ...
        if 'persona' in df.columns:
            ...
            # Take only the first n personas
            selected_personas = []
            for idx in range(min(n, len(df))):
                selected_personas.append((idx, df.loc[idx, 'persona']))
            
            print(f"Selected first {len(selected_personas)} personas from unique_personas.parquet")
            
            # Print details of selected personas
            for i, (idx, persona) in enumerate(selected_personas):
                print(f"Persona {i+1}: ID {idx}, Text: {persona[:100]}...")
...
def run_modified_compas_fairness_study(
    output_file: str = "results/unique_personas_compas_study.csv",
    model: LlamaModel = LlamaModel.LLAMA3_8B,
    temperature: float = 0.7,
    model_path_prefix: str = "../models/",
    comparisons_per_persona: int = 10,
    num_personas: int = 3,
    debug: bool = False
):
    """
    Run a modified study asking the first 2 unique personas to evaluate 10 random COMPAS pairs each.
    ...
    Args:
        ...
        num_personas: Number of unique personas to use
    """
    # Load first n personas from unique_personas.parquet
    personas = load_first_n_personas_from_unique_parquet(num_personas)
    if not personas:
    ...
    with open(output_file, 'a', newline='') as csvfile:
        ...
        for persona_idx, (persona_id, persona_text) in enumerate(personas):
            print(f"\nProcessing persona {persona_idx+1}/{len(personas)}:")
            ...
            for comparison_idx in range(comparisons_per_persona):
                print(f"  Comparison {comparison_idx+1}/{comparisons_per_persona}")
                ...
                try:
                    ...
                    if debug:
                        print(f"\nIndividual A:")
                        print(f"  Sex: {individual1['sex']}, Age: {individual1['age']}, Race: {individual1['race']}")
                        print(f"  Priors: {individual1['priors_count']}, Charge: {individual1['c_charge_degree_full']}")
                        print(f"Individual B:")
                        print(f"  Sex: {individual2['sex']}, Age: {individual2['age']}, Race: {individual2['race']}")
                        print(f"  Priors: {individual2['priors_count']}, Charge: {individual2['c_charge_degree_full']}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run a COMPAS fairness study with the first 3 personas from unique_personas.parquet")
    parser.add_argument("--output", default="results/unique_personas_compas_study.csv", help="Output CSV file")
    ...
    parser.add_argument("--comparisons", type=int, default=10, help="Number of comparisons per persona")
    parser.add_argument("--personas", type=int, default=3, help="Number of personas to use from unique_personas.parquet")
    parser.add_argument("--debug", action="store_true", help="Enable debug output")
    
    args = parser.parse_args()
    
    # Use command line arguments
    print(f"Running COMPAS fairness study with the first {args.personas} personas from unique_personas.parquet")
    ...
    run_modified_compas_fairness_study(
