...
def load_first_n_unique_personas(n=2) -> List[Tuple[int, str]]:
    """
    Load the first n unique personas from the persona_train1.parquet file
    
    Args:
        n: Number of unique personas to load
    ...
    try:
        ...
        if 'persona' in df.columns:
            ...
            selected_personas = []
            for idx, persona in enumerate(unique_personas[:n]):
                selected_personas.append((idx, persona))
            
            print(f"Selected first {len(selected_personas)} unique personas")
...
def run_modified_compas_fairness_study(
    output_file: str = "results/modified_compas_fairness_study.csv",
    model: LlamaModel = LlamaModel.LLAMA3_8B,
    temperature: float = 0.7,
    ...
    """
    # Load first n unique personas
    personas = load_first_n_unique_personas(num_personas)
    if not personas:
    ...
    with open(output_file, 'a', newline='') as csvfile:
        ...
        for persona_idx, (persona_id, persona_text) in enumerate(personas):
            print(f"\nProcessing persona {persona_idx+1}/{len(personas)}: {persona_text[:50]}...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run a modified COMPAS fairness study with 2 personas and 10 comparisons each")
    parser.add_argument("--output", default="results/modified_compas_fairness_study.csv", help="Output CSV file")
    ...
    parser.add_argument("--personas", type=int, default=2, help="Number of unique personas to use")
    
    args = parser.parse_args()
    
    # Use command line arguments
    print(f"Running modified COMPAS fairness study with {args.personas} personas and {args.comparisons} comparisons each")
