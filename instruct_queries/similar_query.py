...
def fairness_query(
    ...
    if persona:
        system_prompt = f"{base_system_prompt} with the persona of {persona}. First, state ONLY ONE of these two exact phrases: \"Should be treated similarly\" or \"Ok to treat differently, or no opinion\". Then, you may provide ONE brief explanation (< 10 words) ."
    else:
        system_prompt = f"{base_system_prompt}. First, state ONLY ONE of these two exact phrases: \"Should be treated similarly\" or \"Ok to treat differently, or no opinion\". Then, you can provide < 10 words for explanation."
