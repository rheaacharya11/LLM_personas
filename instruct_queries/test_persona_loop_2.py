...
def preference_query(
    ...
    if persona:
        ...
        if force_binary:
            system_prompt = f"{base_system_prompt}. Please carefully consider both options before making a decision. You must respond with ONLY the digit 1 or 2 to indicate your preference. No explanation or other text."
        else:
...
def run_preference_study(
    ...
    # Load model and tokenizer once for all personas
    model_path = f"../models/{model.value}"
    print(f"Loading {model.value} model and tokenizer (only once)...")
    tokenizer = AutoTokenizer.from_pretrained(model_path)
    ...
    with open(output_file, 'a', newline='') as csvfile:
        ...
        for i, persona in enumerate(tqdm(personas, desc="Processing personas")):
            ...
            try:
                ...
                # Add persona
                if force_binary:
                    system_prompt = f"{base_system_prompt} with the persona of {persona}. First state your choice as either '1' or '2', then provide exactly one sentence explaining your preference."
                else:
                    system_prompt = f"{base_system_prompt} with the persona of {persona} that says either (1) if you prefer Option 1 or (2) if you prefer Option 2. Then, you give a 1 sentence explanation."
                ...
                # Format the prompt using Llama 3 special tokens
                formatted_prompt = format_llama3_prompt(prompt_list, tokenizer)
                ...
                with torch.no_grad():
                    output = model_instance.generate(
                        input_ids,
                        attention_mask=attention_mask,
                        max_new_tokens=512,  # Using default max_tokens
                        temperature=temperature,
                        top_p=0.9,
                        do_sample=True,
                        pad_token_id=tokenizer.pad_token_id
                ...
                if choice_number is None:
                    if "1" in assistant_response[:10] and "2" not in assistant_response[:10]:
                        choice_number = 1
                    elif "2" in assistant_response[:10] and "1" not in assistant_response[:10]:
                        choice_number = 2
                
                # Add result to batch
                result = {
                    'persona': persona,
                    'choice': assistant_response,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--choice1", default="Vanilla ice cream", help="First choice option")
    parser.add_argument("--choice2", default="Chocolate ice cream", help="Second choice option")
    ...
    parser.add_argument("--max_personas", type=int, default=500, help="Maximum number of personas to use")
    parser.add_argument("--temperature", type=float, default=0.7, help="Sampling temperature")
    parser.add_argument("--batch_size", type=int, default=10, help="Batch size for saving results")
    parser.add_argument("--force_binary", action="store_true", default=False, 
                      help="Force binary choice with no explanation")
    parser.add_argument("--explain", action="store_true", default=True,
                      help="Include a one-sentence explanation with the choice")
    ...
    run_preference_study(
        ...
        force_binary=not args.explain  # If explain is True, force_binary should be False
    )