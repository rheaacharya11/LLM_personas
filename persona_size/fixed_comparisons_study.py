...
def llama_query(
    ...
    if "Should be treated similarly" in assistant_response:
        ...
        choice = "different"
        # Try to extract the explanation that comes after the choice
        parts = assistant_response.split("Ok to treat differently", 1)
        if len(parts) > 1 and parts[1].strip():
            explanation = parts[1].strip()
...
def prepare_compas_data(df: pd.DataFrame, test_size: float = 0.2, random_state: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Clean and prepare the COMPAS dataset according to Jung paper criteria.
    """
    print(f"Original dataset: {len(df)} rows")
    
    # Apply the specific filtering criteria from the Jung paper
    clean_df = df.copy()
    
    # Filter by days between screening and arrest (±30 days)
    clean_df = clean_df[clean_df['days_b_screening_arrest'] <= 30]
    clean_df = clean_df[clean_df['days_b_screening_arrest'] >= -30]
    print(f"After filtering by screening date: {len(clean_df)} rows")
    
    # Remove invalid recidivism indicators
    clean_df = clean_df[clean_df['is_recid'] != -1]
    print(f"After removing invalid recidivism indicators: {len(clean_df)} rows")
    
    # Remove charge degree "O"
    clean_df = clean_df[clean_df['c_charge_degree'] != "O"]
    print(f"After removing charge degree 'O': {len(clean_df)} rows")
    
    # Remove missing risk scores
    clean_df = clean_df[clean_df['score_text'] != "N/A"]
    print(f"After removing missing risk scores: {len(clean_df)} rows")
    
    # Remove any remaining rows with missing values in key fields
    columns_to_check = [
        'id', 'sex', 'age', 'race', 'juv_fel_count', 'juv_misd_count', 
        'juv_other_count', 'priors_count', 'c_charge_degree', 'two_year_recid'
    ]
    clean_df = clean_df[columns_to_check].dropna()
    ...
    train_df, test_df = train_test_split(
        ...
        stratify=clean_df['two_year_recid']
    )
...
def generate_fixed_comparisons(df: pd.DataFrame, num_comparisons: int = 100, random_state: int = 42) -> List[Tuple[pd.Series, pd.Series]]:
    """
    Generate a fixed set of pairwise comparisons from the dataset.
    ...
    Returns:
        List of tuples, each containing two pandas Series (individuals to compare)
    """
    ...
    for i in range(num_comparisons):
        ...
        individual1 = df.iloc[indices[0]]
        individual2 = df.iloc[indices[1]]
        # Add to our list
        comparisons.append((individual1, individual2))
    
    print(f"Generated {len(comparisons)} fixed comparisons")
    return comparisons

def save_comparisons_to_file(comparisons: List[Tuple[pd.Series, pd.Series]], output_file: str = "data/fixed_comparisons.json"):
    """
    Save the fixed comparisons to a JSON file for future reference and reproducibility.
    
    Args:
        comparisons: List of comparison tuples
    ...
    for i, (individual1, individual2) in enumerate(comparisons):
    ...
    print(f"Saved {len(comparisons)} fixed comparisons to {output_file}")

def load_comparisons_from_file(df, input_file="data/fixed_comparisons.json"):
    """
    ...
    for comparison in serialized_comparisons:
        ...
        # Find individuals by ID - more robust approach
        try:
            individual1_rows = df[df['id'] == individual1_id]
            individual2_rows = df[df['id'] == individual2_id]
            
            if len(individual1_rows) > 0 and len(individual2_rows) > 0:
                individual1 = individual1_rows.iloc[0]
                individual2 = individual2_rows.iloc[0]
                loaded_comparisons.append((comparison_id, individual1, individual2))
            else:
                missing_comparisons.append(comparison_id)
        except Exception as e:
            print(f"Error processing comparison {comparison_id}: {e}")
            missing_comparisons.append(comparison_id)
    
    if missing_comparisons:
        print(f"Warning: Could not find individuals for {len(missing_comparisons)} comparisons.")
        print(f"Missing comparison IDs: {missing_comparisons[:10]}{'...' if len(missing_comparisons) > 10 else ''}")
...
def fairness_query(
    ...
    if persona:
        system_prompt = f"{base_system_prompt} with the persona of {persona}. First, state ONLY ONE of these four exact phrases: \"Should be treated similarly\" or \"X at least as high as Y\" or \"Y at least as high as X\" or \"Ok to treat differently, or no opinion\". Then, you may provide ONE brief explanation about main decision factors (< 10 words) ."
    else:
        system_prompt = f"{base_system_prompt}. First, state ONLY ONE of these four exact phrases: \"Should be treated similarly\" or \"X at least as high as Y\" or \"Y at least as high as X\" or \"Ok to treat differently, or no opinion\". Then, you may provide ONE brief explanation about main decision factors(< 10 words)"
...
def run_fixed_comparisons_study(
    output_file="results/fixed_comparisons_study.csv",
    model=LlamaModel.LLAMA3_8B,
    temperature=0.7,
    ...
    end_persona_index=None,
    start_comparison_index=0,
    end_comparison_index=None,
    force_regenerate_comparisons=False,
    random_state=42
):
    """
    ...
    Args:
        ...
        num_comparisons: Number of fixed comparisons to use
        fixed_comparisons_file: Path to save/load fixed comparisons
    ...
    # If we couldn't load enough comparisons, regenerate them
    if len(loaded_comparisons) < num_comparisons * 0.7:  # If we're missing more than 30% of comparisons
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="results/fixed_comparisons_study.csv", help="Output CSV file")
    ...
    parser.add_argument("--comparisons", type=int, default=100, help="Number of fixed comparisons to use")
    ...
    print(f"Each persona will evaluate {args.comparisons} fixed pairs")
