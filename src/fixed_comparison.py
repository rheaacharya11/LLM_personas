...
class FairnessConstraintGenerator:
    ...
    def generate_unique_comparisons(self, train_df, personas_df, fixed_comparisons_path):
        """Generate comparison pairs that are evaluated by multiple personas."""
        import json
        
        if self.verbose:
            print(f"Loading fixed comparisons from {fixed_comparisons_path}")
        
        # Load the fixed comparisons from JSON
        with open(fixed_comparisons_path, 'r') as f:
            fixed_comparisons = json.load(f)
        
        if self.verbose:
            print(f"Loaded {len(fixed_comparisons)} fixed comparison pairs")
            print(f"All personas will evaluate all comparison pairs")
        
        # Create a dictionary to store comparisons for each persona
        persona_comparisons = {}
        
        # Assign all comparisons to each persona
        for i in range(len(personas_df)):
            persona_comparisons[i] = fixed_comparisons
    ...
    def elicit_fairness_judgments(self, train_df, personas_df, 
                        start_comparison=0, end_comparison=None,
                        use_personas=True, prompt_type="chain_of_thought",
                        output_path=None, fixed_comparisons_path=None):
        """
        Elicit fairness judgments using fixed comparisons loaded from a JSON file.
        ...
        Args:
            ...
            fixed_comparisons_path: Path to the JSON file containing fixed comparisons
        """
        if self.verbose:
            print(f"Eliciting fairness judgments for {len(personas_df)} personas")
            print(f"Using fixed comparisons from {fixed_comparisons_path}")
            print(f"Using personas: {use_personas}, Prompt type: {prompt_type}")
            if start_comparison > 0 or end_comparison is not None:
                print(f"Processing comparison range: {start_comparison} to {end_comparison}")
        ...
        # Load comparisons for each persona (all personas get all comparisons)
        persona_comparisons = self.generate_unique_comparisons(
            train_df, 
            personas_df,
            fixed_comparisons_path=fixed_comparisons_path
        )
        
        # Get the total number of comparisons
        total_comparisons = len(persona_comparisons[0]) if 0 in persona_comparisons else 0
        
        # Adjust comparison range for parallelization
        if end_comparison is None:
            end_comparison = total_comparisons - 1
        ...
        for persona_idx, (_, persona_row) in enumerate(personas_df.iterrows()):
            ...
            for comparison_idx in range(start_comparison, min(end_comparison + 1, len(persona_comparison_list))):
                ...
                # Query the model for the normal order
                ...
                if chosen_option2 is not None:
                    if judgment_type == "binary":
                        # Binary judgment mapping
                        if 1 <= chosen_option2 <= 2:
                            judgment2 = "similar" if chosen_option2 == 1 else "different"
                        else:
                            judgment2 = "unknown"
                    else:
                        # Three-option judgment mapping
                        if 1 <= chosen_option2 <= 3:
...
def main():
    ...
    parser.add_argument("--fixed_comparisons_path", type=str, default=None, required=True, help="Path to fixed comparison data")
    ...
    result = generator.elicit_fairness_judgments(
        ...
        fixed_comparisons_path=args.fixed_comparisons_path
    )
