...
class FairnessElicitationAlgorithm:
    ...
    def run(self):
        ...
        print(f"Starting algorithm run with:")
        print(f"  Total constraints: {len(self.constraints)}")
        print(f"  Constraint weights: {len(self.w_ij)} unique weights")
        print(f"  Time horizon: {self.time_horizon} iterations")
...
def main():
    """Main function to run the algorithm with gamma parameter exploration"""
    import argparse
    ...
    parser.add_argument("--eta", type=float, default=0.1,
                        help="Fixed eta (violation budget) value")
    ...
    # More granular and extended range of gamma values
    gamma_values = [
        0.0,   # No buffer for fairness violations
        0.05,  # Very small buffer
        0.1,   # Small buffer
        0.2,   # Moderate buffer
        0.3,   # Larger buffer
        0.4,   # Even larger buffer
        0.5,   # Half the prediction range
        0.75,  # Very large buffer
        1.0    # Maximum buffer
    ]
    
    # Initialize the algorithm with base parameters
    algorithm = FairnessElicitationAlgorithm(
        ...
        eta=args.eta,  # Use the specified eta
        verbose=args.verbose
    )
    
    print(f"Running parameter sweep")
    print(f"Gamma values: {gamma_values}")
    print(f"Fixed eta value: {args.eta}")
    
    # Run parameter sweep with fixed eta
    results = algorithm.run_parameter_sweep(
